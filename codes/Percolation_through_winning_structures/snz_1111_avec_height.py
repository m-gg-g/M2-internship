...
if __name__ == "__main__":
    ...
    np.savetxt('1111_p_array_for' + 'MAX_L' + str(MAX_L) + "-" + str(MAXITER) + str(LLL), p_wanted, delimiter = ' ')
    np.savetxt('1111_L_array_for' + 'MAX_L' + str(MAX_L) + "-" + str(MAXITER) + str(LLL), L_, delimiter = ' ')

    plt.plot(L_, p_wanted)
    plt.xlabel('N')
    plt.ylabel('p found for ' + str(4) + " 1s")
    plt.savefig('1111_L_vs_p_MAX_L' + str(MAX_L) + '-' + str(MAXITER) + str(LLL) + '.png')  
    plt.close()
